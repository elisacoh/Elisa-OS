/*
  # Système de catégories unifié avec protection

  1. Modifications
    - Ajout de la colonne `is_protected` pour protéger certaines catégories
    - Mise à jour des politiques RLS pour permettre la gestion des catégories
    - Protection de la catégorie "Autre" contre suppression/modification
    - Fallback automatique vers "Autre" lors de suppression de catégories

  2. Sécurité
    - Politiques RLS granulaires par opération
    - Protection de la catégorie "Autre" par triggers
    - Gestion automatique des références orphelines

  3. Fonctions
    - `get_fallback_category_id` : obtenir l'ID de la catégorie de fallback
    - `handle_category_deletion` : gérer les suppressions avec fallback
    - `prevent_protected_modification` : empêcher modification des catégories protégées
    - `get_user_categories` : obtenir les catégories d'un utilisateur
*/
...
-- Nouvelles politiques RLS - SELECT
...
-- Nouvelles politiques RLS - INSERT
...
-- Nouvelles politiques RLS - UPDATE
...
-- Nouvelles politiques RLS - DELETE
...
-- Recréer la fonction get_user_categories avec le nouveau type de retour
