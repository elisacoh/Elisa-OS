/*
  ...
  1. Nettoyage complet
    ...
    - Suppression de tous les triggers existants
  ...
  2. Nouveau système simplifié
    ...
    - Politiques RLS simples et claires

  3. Sécurité
    - Protection absolue de "Autre" uniquement
    - Redirection automatique vers "Autre" lors des suppressions
    - Permissions claires pour les utilisateurs
*/
...
UPDATE categories SET is_protected = false;
...
CREATE POLICY "Users can update non-protected categories"
  ...
  TO authenticated
  USING (
    NOT (name = 'Autre' AND COALESCE(is_protected, false) = true)
  )
  WITH CHECK (
    NOT (name = 'Autre' AND COALESCE(is_protected, false) = true)
  );
